...
-- 2017-01-01,NAMEXXX,Created File
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_NAMEXXX')
	 Begin 
	  Alter Database [Assignment07DB_NAMEXXX] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_NAMEXXX;
	 End
	Create Database Assignment07DB_NAMEXXX;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_NAMEXXX;
...
SELECT 
    p.ProductName,
    FORMAT(i.InventoryDate, 'MMMM', 'en-US') + ', ' + DATENAME(YY, i.InventoryDate) AS InventoryDate,
    i.Count
FROM vProducts AS p 
...
-- Note: Changed Product sorting to Category sorting.
...
CREATE VIEW vCategoryInventories AS
    SELECT TOP 100000000
        c.CategoryName,
        FORMAT(i.InventoryDate, 'MMMM', 'en-US') + ', ' + DATENAME(YY, i.InventoryDate) AS InventoryDate,
        SUM(i.Count) AS InventoryCountPerCategory
...
CREATE VIEW vProductInventoriesWithPreviouMonthCounts AS
    SELECT TOP 100000000
        ...
        InventoryDate,
        Count,
        IIF(InventoryDate LIKE ('Janu%'), 0, ISNULL(LAG(Count) OVER(ORDER By ProductName, YEAR(InventoryDate)), 0)) AS PreviousMonthCount
    FROM vProductInventories
    ORDER BY ProductName, MONTH(InventoryDate), PreviousMonthCount;
GO
